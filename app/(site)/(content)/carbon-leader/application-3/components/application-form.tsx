...
// 선도기업 신청 3차 STEP 1(신청서 작성).
// 「중간 및 최종점검 신청서」 양식이라 카드 5장이 같은 껍데기를 쓴다.
// 1·2차와 달리 기준연도 현황·감축계획 카드가 없고, 1차에 적어 둔 투자계획을
// 읽기 전용으로 되짚은 뒤 감축기술 도입현황을 새로 받는다.
// 표시 값은 전부 고정 문구다. constants/carbon-leader-application-form 참고.
const NOTICES = [
  "기업 기본정보를 확인하고, 탄소중립 투자계획 및 감축기술 도입현황을 입력해 주세요. 관리자 검토 후 인증서가 발급됩니다.",
]
...
const Card = ({ id, title, description, action, children }: CardProps) => (
  <section
    id={id}
...
/** 카드 헤더의 테두리 버튼. 지금은 [내 정보 불러오기] 하나가 쓴다 */
const HeaderButton = ({
  children,
  icon,
  loading = false,
    ...
    <div className="flex w-full max-w-316 flex-col md:gap-8 md:px-7 md:pt-12 md:pb-28 lg:gap-10 lg:px-8 lg:pt-14 lg:pb-42">
      ...
      <div className="flex flex-col gap-8 max-md:hidden lg:flex-row lg:items-start lg:justify-between lg:gap-8">
        ...
        <div className="[&_ol]:flex-nowrap sm:w-full sm:[&_ol]:w-full sm:[&_ol>li]:flex-1 sm:[&_ol>li>div:nth-child(1)]:flex-1 sm:[&_ol>li>div:nth-child(3)]:flex-1 lg:w-152">
          <Stepper items={APPLICATION_THIRD_STEPS} activeIndex={0} size={13} />
        </div>
      </div>
