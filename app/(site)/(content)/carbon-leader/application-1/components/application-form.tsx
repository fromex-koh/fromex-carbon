...
const ApplicationForm = ({
  ...
  // (6) 표에 세울 줄. 1차는 감축잠재량 산정에서 넘어온 목록을 그대로 쓰고,
  // 2차는 빈 줄 하나로 시작해 사용자가 채우고 늘린다.
  ...
  const rows = isSecond ? investRows : INVESTMENT_ROWS
      ...
      <FieldErrorContext.Provider value={errors}>
        ...
        >
          ...
          <Card
            ...
            description={
              isSecond
                ? "탄소중립 투자계획을 입력합니다. 행 추가 버튼으로 항목을 자유롭게 추가할 수 있습니다."
            ...
            <div className={cn("flex flex-col", TABLE_BOX)}>
              ...
              <div className="bg-surface-disabled text-ink-muted hidden gap-8 px-8 text-xs font-bold xl:flex">
                ...
                <div className="flex flex-1 gap-2.5">
                  ...
                  {/* 삭제 버튼 자리 */}
                  {isSecond ? <span className="w-21 shrink-0" /> : null}
                </div>
              </div>
              ...
              <div className="divide-line-card flex flex-col divide-y md:p-6 lg:p-8 xl:gap-2.5 xl:divide-y-0">
                {rows.map((row, index) => (
                  <div
                    key={row.code}
                    // 줄을 늘린 뒤 창을 따라 옮길 때 이 줄을 찾는다(2차)
                  ...
                  >
                    ...
                    <div className="grid gap-4 md:grid-cols-2 md:gap-x-2.5 xl:flex xl:flex-1 xl:items-start xl:gap-2.5">
                      {INVESTMENT_COLUMNS.slice(0, 2).map((column) => (
                        ...
                        >
                          {/* 1차는 감축잠재량 산정에서 넘어오는 값이라 읽기 전용이다 */}
                          <input
                            ...
                            className={cn(
                              FIELD,
                              !isSecond && "bg-surface-disabled text-ash-500",
                          ...
                          <UnitField
                            ...
                            {...(isSecond
                              ? {
                                  invalid:
                                    !!errors[
                                      `investment-${row.code}-${column.key}`
                                    ],
                                }
                              : {
                                  value:
                ...
                {/* 시안 PC_add_BTN 규격은 (7) 표의 [행 추가하기] 와 같다 */}
                {isSecond ? (
                  <button
