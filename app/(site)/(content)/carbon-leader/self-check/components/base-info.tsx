...
const BaseInfo = ({ items }: { items: string[] }) => {
  return (
    <section className="bg-ash-100 flex flex-col gap-3 px-6 pt-6 pb-8 max-md:rounded-none md:rounded-2xl lg:px-10">
      <h3 className="text-lg font-bold lg:text-xl">시작하기전에</h3>
      <ul className="flex flex-col gap-2">
        {items.map((item) => (
          <li
            key={item}
            className="text-ash-800 flex gap-1 text-base break-all"
          >
            <span
              aria-hidden="true"
