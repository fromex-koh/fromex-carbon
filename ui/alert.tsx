  ...
  "relative grid w-full grid-cols-[0_1fr] items-start gap-y-0.5 rounded-lg px-4 py-3 text-sm has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] has-[>svg]:gap-x-3 [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        ...
        info: "rounded-2xl bg-ash-100 px-10 py-6",
      },
    },
    defaultVariants: {
      variant: "default",
...
function AlertTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "col-start-2 line-clamp-1 min-h-4 text-lg font-bold font-medium tracking-tight",
        className,
      )}
      {...props}
    />
...
function AlertDescription({
  ...
  return (
    <div
      ...
      className={cn(
        "col-start-2 grid justify-items-start gap-1 text-ash-800 [&_p]:leading-relaxed",
        className,
      )}
      {...props}
    />
