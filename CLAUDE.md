...
**git commit 과 git push 는 사용자가 명시적으로 요청하거나 승인했을 때만 실행한다.**

- 작업을 끝냈다고 해서 자동으로 커밋하지 않는다. 변경 내용을 요약해 보고하고 거기서 멈춘다.
- "~해줘", "~만들어줘" 같은 작업 지시는 **파일 수정까지만** 포함한다. 커밋·푸시 승인이 아니다.
- 직전 대화에서 커밋을 승인받았더라도 다음 작업에는 적용되지 않는다. 매번 다시 묻는다.
- 공개 저장소이고 팀이 함께 쓰는 브랜치이므로, 올릴 시점과 커밋 단위는 사용자가 정한다.
